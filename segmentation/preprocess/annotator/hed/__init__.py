...
from preprocess.annotator.util import annotator_ckpts_path
...
class HEDdetector:
    ...
    def batch_edge(self, input_image):
        with torch.no_grad():
            image_hed = torch.from_numpy(input_image).float().cuda()
            image_hed = image_hed / 255.0
            image_hed = rearrange(image_hed, 'n h w c -> n c h w')
            edge = self.netNetwork(image_hed)
            edge = (edge.cpu().numpy() * 255.0).clip(0, 255).astype(np.uint8)
            return edge
